    ...
    def agm(self,inicio):
        ...
        while len(pilha) != 0:
            ...
            if v == -1:
                ...
                self.listaVertices[v].regVisitado()
                pilha.append(v)
                self.Mostra_Vertice(elementoAnalisar)
                self.Mostra_Vertice(v)
                print( " ",end='')
        for i in self.listaVertices:
            i.Limpa()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("(m) Matriz, (v) Vertice (a) Arco (b)DFS (l)BFS (g)AGM (s) Sair")
        ...
        if escolha == 'm':
            ...
            r_inicio = input('Informe o rótulo de início da AGM: ')
            inicio = grf.Localiza_Rotulo(r_inicio)
            if inicio == -1:
