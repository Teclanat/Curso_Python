...
class Grafo:
    ...
    def Remove_Vertice(self,vertice):
        for linha in range(len(self.matrizAdjacencias)):
            del self.matrizAdjacencias[linha][vertice]
        del self.matrizAdjacencias[vertice]
        del self.listaVertices[vertice]
        self.numVertices -= 1


    def Topo(self):
        while(self.numVertices > 0):
            verticeAtual = self.VerticeSemSucessor()
            if verticeAtual == -1: #Ciclo
                print('Erro! O grafo possui ciclos')
                return
            self.matrizOrdenada.append(self.listaVertices[verticeAtual].rotulo)
            self.Remove_Vertice(verticeAtual)
        self.matrizOrdenada.reverse()
        print('Lista de Vértices Ordenados Topologicamente: ', self.matrizOrdenada)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("(m) Matriz, (v) Vertice (a) Arco (t)Topologica (s) Sair")
